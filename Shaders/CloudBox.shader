	...
	SubShader
	{
		...
		Pass
		{
			...
			float calculateDensity(float3 target, float height)
			{
				...
				float cloud_coverage = pow(coverage, remap(height, 0.7f, 0.8f, 1.0f, lerp(1.0, 0.5, 0.5)));
				float base_shape_with_coverage = remap(base_shape_with_cloud_types, cloud_coverage, 1.0, 0.0, 1.0);
				...
				float high_freq_modifier = mix(high_freq_fbm, 1.0 - high_freq_fbm, clamp(height * 10.0, 0.0, 1.0));

				//base_shape_with_coverage = base_shape_with_coverage - high_freq_modifier * (1.0 - base_shape_with_coverage);
				//base_shape_with_coverage = remap(base_shape_with_coverage * 2.0, high_freq_modifier * 0.2, 1.0, 0.0, 1.0);
				//base_shape_with_coverage = remap(base_shape_with_cloud_types, high_freq_modifier * 0.2, 1.0, 0.0, 1.0);

				float edge_y = remap(height * 2, 0.0, 1.0, 0.5, 0.0);

				base_shape = remap(base_shape, high_freq_fbm - 1, 1.0, 0.0, 1.0);
				//final
				float final = clamp(base_shape_with_coverage, 0.0, 1.0);



				final = max(0, base_shape - _DensityThreshold) * _ShapeDensityStrength * edge_y;// *rate_x* rate_z* rate_y;
				return final;
			}
			...
			float4 calculateFinalColor(in float3 rayOrg, in float3 rayDir, in float3 lightDir, in float depth, in float2 uv)
			{
				...
				while (total_thickness < cloud_thickness)
				{
					...
					{
						float light_transmittance = calculateLightTransmittance(p, lightDir, height);
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				float3 cloud_col = col_params.xyz * _LightColor0 + (1 - col_params.w) * UNITY_LIGHTMODEL_AMBIENT.xyz;
